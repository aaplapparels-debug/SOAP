...
class TallyAdapter:
    ...
    def _extract_bill_references(self, voucher) -> list:
        ...
        Also captures WHICH ledger entry each bill allocation came from
        (party_ledger_name) -- necessary because a single voucher can
        settle bills across multiple different party ledgers at once
        (confirmed in testing: one bank receipt split across three
        related customer accounts). The voucher-level PARTYLEDGERNAME
        field only ever captures one of them, so each bill reference
        needs to be attributed to its own specific owning ledger, not
        a single shared name for the whole voucher.
        """
        refs = []
        ledgers = (
    ...
    def _extract_payment_details(self, voucher) -> dict:
        """
        Extracts payment mode, bank name, instrument number, and instrument date.
        Identifies payment ledgers by checking for genuine, non-empty bill allocations
        (ignoring empty <BILLALLOCATIONS.LIST> tags emitted by Tally).
        """
        ledgers = (
            voucher.findall('.//ALLLEDGERENTRIES.LIST') 
            + voucher.findall('.//LEDGERENTRIES.LIST')
        ...
        payment_ledger = None
        for ledger in ledgers:
            # Check if this ledger contains real bill allocations with non-empty reference names
            has_real_bill_alloc = any(
                b.findtext('NAME', '').strip() != ''
                for b in ledger.findall('.//BILLALLOCATIONS.LIST')
            )
            if has_real_bill_alloc:
                continue  # Skip party/customer ledgers
        ...
        payment_ledger_name = (
            payment_ledger.findtext('LEDGERNAME', '').strip() 
            if payment_ledger is not None else None
        )

        # Locate BANKALLOCATIONS in payment ledger or voucher root
        bank_alloc = None
        if payment_ledger is not None:
            bank_alloc = payment_ledger.find('.//BANKALLOCATIONS.LIST')
        if bank_alloc is None:
            bank_alloc = voucher.find('.//BANKALLOCATIONS.LIST')
        ...
        if bank_alloc is not None:
            ...
            bank_name = (
                bank_alloc.findtext('BANKNAME', '').strip()
                or bank_alloc.findtext('BANKERSNAME', '').strip()
            )

        # Fallback to voucher-level instrument tags if bank_alloc lacked values
        if not instrument_no:
            inst_elem = voucher.find('.//INSTRUMENTNUMBER')
            if inst_elem is not None and inst_elem.text:
                instrument_no = inst_elem.text.strip()

        if not instrument_date_raw:
            date_elem = voucher.find('.//INSTRUMENTDATE')
            if date_elem is not None and date_elem.text:
                instrument_date_raw = date_elem.text.strip()

        # Determine payment mode and bank ledger name
        if payment_ledger_name:
            mode_of_payment = payment_ledger_name
        elif bank_alloc is not None:
            mode_of_payment = bank_name or "Bank"
        else:
        ...
        # Fall back to ledger name if bank_name is empty on a bank entry
        if not bank_name and mode_of_payment and mode_of_payment.lower() != 'cash':
            bank_name = mode_of_payment

        return {
            'mode_of_payment': str(mode_of_payment)[:50] if mode_of_payment else None,
            'instrument': str(instrument_no).strip()[:50] if instrument_no else None,
            'instrument_date': instrument_date,
            'bank': str(bank_name).strip()[:50] if bank_name else None,
        }
    ...
    def extract_receipts(self) -> pd.DataFrame:
        ...
        for v in vouchers:
            ...
            payment = self._extract_payment_details(v)
            ...
            if not bill_refs:
                ...
                for ref in bill_refs:
                    rows.append({
                        'receipt_date': vch_date, 'voucher_number': vch_no,
                        # Prefer the specific ledger this bill actually
                        # belongs to (correct for multi-party vouchers);
                        # fall back to the voucher-level party only if
                        # that ledger name somehow came through empty.
                        'customer_name': ref['party_ledger_name'] or party,
                        'bill_reference': ref['bill_reference'],
...
if __name__ == "__main__":
    ...
    print(receipts_df[receipts_df['voucher_number'] == '140'][['customer_name', 'bill_reference', 'amount', 'mode_of_payment', 'bank', 'instrument', 'instrument_date']])
