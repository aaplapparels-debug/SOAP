...
class ShoperAdapter:
    ...
    SOURCE_SYSTEM = 'shoper'
    def __init__(self, divisions: List[DivisionConfig], financial_years_back: int, sales_orders_lookback_days: int):
        ...
        # The cutoff used for a full backfill of sales/purchases (when
        # since_date is left as None). Customers and items aren't
        # filtered by this at all -- they're current master data, not
        # history, so we always want all of it.
        self.default_cutoff = get_financial_year_cutoff(datetime.date.today(), financial_years_back)
        # Sales orders use a separate, fixed rolling window -- see
        # extract_sales_orders below.
        self.sales_orders_lookback_days = sales_orders_lookback_days

    def _get_engine(self, division: DivisionConfig):
        """Create a SQLAlchemy engine for one division's database."""
        from urllib.parse import quote_plus
        
        # URL-encode the password so special characters don't break the connection string
        encoded_password = quote_plus(division.password)
        
        connection_url = (
            f"mssql+pyodbc://{division.username}:{encoded_password}@"
            f"{division.server}/{division.database}?"
    ...
    def _run_for_all_divisions(self, query: str) -> pd.DataFrame:
        ...
        for division in self.divisions:
            engine = self._get_engine(division)
            try:
                df = pd.read_sql(text(query), engine)
                df["division"] = division.division_name
                row_count = len(df)
                print(f"  └─ {division.division_name}: {row_count} rows")
                frames.append(df)
            except Exception as e:
                print(f"  └─ {division.division_name}: ERROR - {e}")
            finally:
                engine.dispose()
    ...
    def extract_customers(self) -> pd.DataFrame:
        query = """
            SELECT Code AS customer_code, Nm AS customer_name,
                   ...
                   CreditUsed AS credit_used, 'shoper' AS source_system
            FROM Customers
        """
    ...
    def extract_items(self) -> pd.DataFrame:
        query = """
            SELECT im.StockNo AS item_code, im.ItemDesc AS item_desc,
                   ...
                   sm.CurBalQty AS stock_qty, sm.CurBalVal AS stock_value, 'shoper' AS source_system
            FROM ItemMaster im
    ...
    def extract_sales_orders(self) -> pd.DataFrame:
        ...
        #
        # Both conditions required together (AND, not OR): an order must
        # be BOTH currently pending AND placed within the last
        # sales_orders_lookback_days days. An order pending for months
        # isn't actionable -- the customer just places a fresh order
        # instead of chasing an old one -- so it's deliberately excluded
        # even though it's technically still "pending" in Shoper.
        #
        # This is a full-refresh snapshot (loader TRUNCATEs before
        # reloading), so an order that gets billed, or ages out of the
        # window, simply won't appear next time -- no stale rows left
        # behind, no separate mechanism needed to detect that.
        cutoff = datetime.date.today() - datetime.timedelta(days=self.sales_orders_lookback_days)
        query = f"""
            SELECT OrderNo AS order_id, CustCd AS customer_code,
                   ...
                   InvValue AS invoice_value, 'shoper' AS source_system
            FROM CustSFASalesOrderDtls
            WHERE PendingQty > 0
              AND OrderDate >= '{cutoff.strftime('%Y-%m-%d')}'
        """
        return self._run_for_all_divisions(query)
    ...
    def extract_purchases(self, since_date: Optional[datetime.date] = None) -> pd.DataFrame:
        ...
        query = f"""
            SELECT h.PartyStkDocNo AS vendor_invoice_no,
                   ...
                   d.StkUpdtRate AS rate, d.DocEntNetValue AS net_value, 'shoper' AS source_system
            FROM StkTrnHdr h
            JOIN StkTrnDtls d
...
if __name__ == "__main__":
    ...
    print("=== Sales Orders (pending AND within lookback window) ===")
