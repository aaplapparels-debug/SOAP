...
class ShoperAdapter:
    ...
    def __init__(self, divisions: List[DivisionConfig], financial_years_back: int, sales_orders_days_back: int):
        ...
        self.sales_orders_days_back = sales_orders_days_back
	print(f"Sales Order Config: {sales_orders_days_back}")
	...
	# Inside extract_sales_orders():
        
        # Default to last 10 days instead of 3 financial years
        since_date = datetime.date.today() - datetime.timedelta(days=self.sales_orders_days_back)
        cutoff = since_date if since_date else self.default_cutoff
	print(f"DEBUG: Active order cutoff date is -> {since_date}")
	print(f"DEBUG: Active order cutoff date is -> {cutoff}")
        query = f"""
